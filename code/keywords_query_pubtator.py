	...
	try:
		...
		for name,fname in options:
			if name in ('-h'):
				usage()
			if name in ('-i'):
				target_pubtator = open(fname,encoding='utf-8')
				whole=0
				for line in target_pubtator:
					if line[8:11] == '|t|':

						whole+=1
				target_pubtator.close()
				target_pubtator = open(fname,encoding='utf-8')
			if name in ('-w'):
				wordfile= open(fname,encoding='utf-8')
			if name in ('-o'):
				sent_report = open(fname,'w',encoding='utf-8')
				filter_sent()
				target_pubtator.close()
				sent_report.close()
